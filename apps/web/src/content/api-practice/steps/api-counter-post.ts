  ...
  try {
    ...
    setSubmitting(false);
      ...
      conditions: [
        {
          id: 'stringify-body',
          label: 'JSON.stringify でリクエストボディを作っている',
          requiredKeywords: ['JSON.stringify'],
          explanation: 'fetch の body には JSON.stringify した文字列を渡します。',
        },
      ],
      explanation: 'PUT /counterにJSON.stringifyしたボディを送り、finallyでsubmittingをfalseに戻すのが送信処理の基本パターンです。',
      solutionCode: `body: JSON.stringify({ value: value + 1 }),`,
    },
    challengeTask: {
      primaryPatternId: 'c1',
      patterns: [
        {
          ...
          conditions: [
            {
              id: 'put-fetch',
              label: 'PUT /counter で値を更新している',
              requiredKeywords: ['fetch', 'PUT'],
              explanation: 'カウンターの値更新は PUT メソッドで /counter に送信します。',
            },
            {
              id: 'json-body',
              label: 'JSON ボディで nextValue を送っている',
              requiredKeywords: ['JSON.stringify'],
              explanation: 'nextValue を { value: nextValue } として JSON.stringify します。',
            },
            {
              id: 'disable-buttons',
              label: '送信中にボタンを disabled にしている',
              requiredKeywords: ['disabled'],
              explanation: 'submitting 中は disabled を付け、連打による重複送信を防ぎます。',
            },
          ],
          solutionCode: `async function sendValue(nextValue: number) {
  setSubmitting(true);
  try {
    const res = await fetch('/counter', {
      method: 'PUT',

...
| 参照元仕様 | `Specification.md` v2.0 を基に、現状実装へ合わせて調整した v1 仕様 |
| MVP仕様書 | `coden-MVP.md`（フェーズAの詳細はそちらを参照） |
| この版で扱わないもの | ソーシャルログイン、マスコット設定、今日の目標、アナリティクス基盤 |
...
| 要素 | 仕様 |
|------|------|
| ウェルカムバナー | ユーザー名入りパーソナライズ挨拶 + 固定マスコット表示 |
| 学習進捗カード | 現在のコース名・完了ステップ数・全体進行率（%） |
| 復習リスト | Practice / Test で不正解だったステップを端末ローカル（`localStorage`）に保存して表示。端末・ブラウザ単位の機能として扱い、Supabase 同期は行わない |
| 学習ヒートマップ | GitHubスタイルのカレンダー（過去30日間の学習履歴） |
| 学習統計 | 合計ポイント・連続学習日数・今月の学習日数を表示 |
| 学習導線 | 最初の実装済みレッスンへ進むCTAを表示 |
...
| 要素 | 仕様 |
|------|------|
| 認証方式 | メール/パスワード（必須）。ソーシャルログインは v1 スコープ外 |
...
| 要素 | 仕様 |
|------|------|
| 学習統計 | 総ポイント・連続学習日数・最大ストリーク・完了ステップ数・最終学習日を表示 |
| Pt履歴 | 獲得日時・理由・Pt数の一覧 |
| 表示名編集 | プロフィール表示名の更新ができる |
| 実績バッジ | 獲得済み（カラー表示）と未獲得（グレーアウト）の一覧 |
| マスコット表示 | ダッシュボード上で固定表示（ユーザー設定なし） |
...
| 機能 | 保存先 | 同期方針 |
|------|--------|---------|
| 復習リスト（`reviewList`） | ブラウザの `localStorage` | 同一端末・同一ブラウザ内のみ保持。ローカル専用機能として扱い、Supabase への同期は行わない |
...
> KPI は v1 リリース後の運用目標として扱う。計測基盤の導入はリリース後タスクとし、v1 リリース受け入れ基準には含めない。
...
- [ ] 認証（メール/パスワード）・ルート保護・セッション復元が動作
- [ ] 学習進捗・統計・Pt・実績・回答履歴が Supabase に永続化される
- [ ] 模擬API演習（CRUD操作）の成功時UI・失敗時UI・ローディングUIが動作
- [ ] ダッシュボードの主要ウィジェット（進捗・復習リスト・ヒートマップ・学習ステータス）が動作
- [ ] プロフィール画面の統計・表示名編集・Pt履歴・実績一覧が機能
...
| 学習ヒートマップ | 過去30日間の学習履歴UI |
| トースト通知 | 実績解除時のアニメーション付き通知 |
| プロフィール画面 | Pt履歴・実績一覧・表示名編集を実装 |
| ダッシュボード完成 | 進捗・復習リスト・ヒートマップ・学習ステータスの完成 |
...
| 実DB / RLS確認 | 手動テストチェックリストでの実DB確認と証跡整理 |

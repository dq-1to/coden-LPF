...
| # | タスク | 担当 | 理由 |
|---|--------|------|------|
| T8 | ~~BaseNookSidebar コンポーネント新規作成~~ | `[Opus]` | [x] 新規コンポーネント + LearningSidebar パターン踏襲 |
| T9 | ~~BaseNookPage を2カラム + Pagination に変更~~ | `[Opus]` | [x] レイアウト変更 + Pagination 統合 |
| T10 | ~~M4 CI 確認~~ | `[Opus]` | [x] 既存テスト PASS |
...
- [x] T3: ~~サークルが h-8 w-8 / sm:h-10 sm:w-10 に変更済み~~ `[Sonnet]`
- [x] T4: ~~CI 全通過~~ `[Sonnet]`

### M3: Pagination 共通コンポーネント

- [x] T5: ~~Pagination コンポーネントが新規作成済み~~ `[Opus]`
- [x] T6: ~~CodeDoctorPage に Pagination 統合済み~~ `[Opus]`
- [x] T7: ~~CI 全通過~~ `[Opus]`

### M4: BaseNook 2カラム化 + サイドバー TOC

- [x] T8: ~~BaseNookSidebar コンポーネントが新規作成済み~~ `[Opus]`
- [x] T9: ~~BaseNookPage が2カラム + Pagination に変更済み~~ `[Opus]`
- [x] T10: ~~CI 全通過~~ `[Opus]`

### M5: QA

- [x] T11: ~~全対象ページの Playwright スクリーンショット保存~~ `[Sonnet]`
- [x] T12: ~~CI 最終確認~~ `[Sonnet]`

### 品質ゲート

- [x] `typecheck` / `lint` / `test` / `build` 全通過（各マイルストーン完了時）
- [x] 既存テスト全 PASS 維持（696件）
- [x] Playwright で 375px / 390px / 768px / 1280px のスクリーンショット検証（M5 完了時）
- [x] PC レイアウトに変更がないことを 1280px で確認

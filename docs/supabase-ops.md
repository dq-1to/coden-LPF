...
| 種別 | パス |
|---|---|
| 管理対象SQL | `apps/web/supabase/sql/*.sql` |
...
seed / cleanup SQL は固定アカウント、固定パスワード、実データ、一時削除対象を含み得るため Git 管理外にする。スキーマ、RLS、policy、trigger、function、RPC など本番DB構造を再現するための SQL のみ Git 管理対象とする。
...
- 1ファイル1目的を基本にする
- seed / cleanup SQL はこの連番に含めず、Git 管理外の `apps/web/supabase/sql/local/*.sql` または各環境の安全な保管場所で管理する
...
2. 次番号でSQLファイルを作る。
   ```text
   apps/web/supabase/sql/025_example_schema_change.sql
   ```

   seed / cleanup SQL は `apps/web/supabase/sql/` 直下の管理対象SQLとして追加しない。
...
- E2E用固定アカウント seed も Git 管理外にし、必要な環境にだけ手動で投入する
- cleanup SQL は本番・検証環境ごとの一時作業として扱い、Git管理しない

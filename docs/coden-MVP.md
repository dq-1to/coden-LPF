...
> **ステータス**: MVP開発用 確定仕様
> **🎉 MVP開発完全完了**: 2026-02-20

---

## 0. 現在の位置づけ

- この文書は **2026-02-20 時点の MVP ベースライン** を固定するための仕様書。
- 2026-03-10 時点の現行アプリは、この MVP を超えて以下を実装済み:
  - 全4コース・全20ステップ
  - Pt / 実績 / ストリーク
  - プロフィール画面
  - API 連携コース
- したがって、現行実装の判断基準としては `coden-v1.md` を優先し、この文書は「MVPではどこまでを最小成立条件としていたか」を確認する用途で参照する。
...
>
> **ソーシャルログインとマスコット設定の扱い**: このフェーズでは実装しない。メール/パスワード認証を MVP の唯一の認証方式とし、マスコットは固定表示があっても装飾扱いで、ユーザー設定機能は持ち込まない。
...
- [x] `npm run dev` でローカル起動できる（コンソールにエラーなし）
- [x] `/login` `/` `/step/usestate-basic` の3ルートに遷移できる
- [x] 環境変数未設定時に白画面でなく `ConfigErrorView` が表示される
- [x] `profiles` と `step_progress` テーブルが存在し、RLSポリシーが適用済み
- [x] テストユーザー2件が `auth.users` に存在する
...
- [x] テストユーザーでログインし、ダッシュボードが表示される
- [x] 未認証で `/` にアクセスすると `/login` へリダイレクトされる
- [x] サイドバーから4ステップを選択できる
- [x] 閲覧・練習・テスト・チャレンジの全タブが動作する
- [x] 各モードの完了状態が Supabase に保存される
- [x] リロード・再ログイン後も完了状態が保持される
- [x] ダッシュボードの完了ステップ数が更新に追随する
- [x] 別ステップへ移動したとき前ステップの入力・判定フラグがリセットされる
